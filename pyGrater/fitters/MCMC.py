...
#%%
class MCMC:
    def __init__(self, obs_obj, grain, star, density_distribution, size_distribution, phase_function, wavelengths_for_calc, params, n_walkers, n_iters, N_threads=6, covariance_matrix = None, pixAU = 0.002):
        
        self.obs_obj = obs_obj
        
        data_dic = obs_obj.data
        self.cov_matrix = covariance_matrix 

        if not covariance_matrix is None:
            self.corr_mat_bool = True
            print('Taking the covariance matrix into account for the LL (using Moore-Penrose pseudo inverse)')
            self.inverse_cov_matrix = np.linalg.pinv(covariance_matrix)           
        else:
            self.corr_mat_bool = False
        
        nx, ny = 128, 128  
        self.pixAU = pixAU
        self.img_obj = Image(grain, star, density_distribution, size_distribution, 
                        phase_function, wavelengths_for_calc, nx, ny, self.pixAU)

        
        self.B_u = data_dic['u']
        self.B_v = data_dic['v']
        self.waves = data_dic['wave_vis']
        
        self.u = self.B_u/self.waves
        self.v = self.B_v/self.waves
        
        self.obs = data_dic['Vis2']
        self.obs_err = data_dic['Vis2_err']
        # self.ins_V2 = data_dic['INS_V2']
        # self.ins_T3 = data_dic['INS_T3']
    ...
    def model(self, params_dic):
        image = self.img_obj.get_image(keep_separate_fluxes=False, **params_dic)
        # print(image.shape)
        # print(params_dic['r0'])
        # plt.imshow(image[0,:,:])
        # plt.colorbar()
        ...
        image = image[0, :, :]
        # image = images_sca[0, :, :] + images_therm[0, :, :]
        vis2 = transform_image_to_visibilities(image, self.u, self.v, self.waves[0], self.pixAU, padding_factor=10)

        # print(image.shape)
        return np.array(vis2)
    ...
    def log_likelihood(self,theta, kwargs):
        ...
        if self.cov_matrix is None: 
            ...
            LL = -(1/2)*np.matmul(np.matmul((self.obs-model_vis2).T, self.inverse_cov_matrix), (self.obs-model_vis2))
...
if __name__=='__main__':
    ...
    path_test_obs = '/Users/prioletp/PhD/public_codes/pyGrater/pyGrater/fitters/data_tests/TEST_model.fits'
    obs_obj = obs.Observations(path_obs=path_test_obs)
    wave_range = [(3.2e-6, 3.8e-6)]
    obs_obj.filter_data(wave_ranges=wave_range)
    # V2_err = observations.data['Vis2_err'][observations.data['Vis2_err']<0]
    # plt.plot(np.arange(0,len(V2_err), 1), V2_err)
    # print(np.min(V2_err))
    obs_obj.plot(v2_ylim=(0, 1), cp_ylim=(-50, 50))
        
    #%%
    pixAU = 0.005
    test_params = {
        'r0': (0.01, 0.4), 'h0': 0.009, 'alphain': 10., 'alphaout': -6, 
    ...
    test_params_2 = {
        'r0': 0.2, 'h0': 0.009, 'alphain': 10., 'alphaout': -6, 
        'gamma': 2., 'beta': 2, 'itilt': 45., 'PA': 90., 'omega': 45.,
        'a_min': 0.01e-6, 'a_max': 1000e-6, 'kappa': 6, 
        'N_sizes_integral': 200, 'g': 0.5, 'M_tot': 2.5e-10
    }
    ...
    mcmc_fitter = MCMC(obs_obj, grain, star, two_power_law, power_law_distribution, HenveyGreenstein, np.array([3.5]),test_params, n_walkers=20, n_iters=100, N_threads=16, covariance_matrix = None, pixAU = pixAU)
    
    image = mcmc_fitter.img_obj.get_image(keep_separate_fluxes=False, **test_params_2)
    plt.imshow(image[0,:,:])
    plt.colorbar()
    plt.show()
    plt.clf()
